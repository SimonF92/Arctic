...
class Buoy:
    ...
    def processbuoy(self):
        ...
        for value in range (0,(number_of_days-10)):
            ...
            thermistors_not_in_ice=df_HEAT.where(df_HEAT.Deltacore>0.5)
            thermistors_not_in_ice=thermistors_not_in_ice.dropna()
            Th_ice= thermistors_not_in_ice.tail(1)
            Th_ice=Th_ice.index.values.tolist()
            #print(Th_ice)
            #Th_ice=int(Th_ice[0])
            ...
            ice_thermistors=ice_or_water_thermistors.where(ice_or_water_thermistors.mean_t<(core_temperature))
            ...
            df_HEAT_subset=df_HEAT_main.iloc[(i-20):i]
            df_HEAT_subset=df_HEAT_subset.drop(['time', 'latitude (deg)','longitude (deg)'], axis=1)
            df_HEAT_subset.loc['stdev'] = df_HEAT_subset.std()
            df_HEAT_subset.loc['mean_t'] = df_HEAT_subset.mean()
            df_HEAT_subset=df_HEAT_subset.tail(2)
            df_HEAT_subset=df_HEAT_subset.T
            df_HEAT_subset=df_HEAT_subset.head(-5)
            df_HEAT_subset.reset_index(level=0, inplace=True)
            total_thermistors_in_column=len(df_HEAT_subset)
            ice_or_water_thermistors=df_HEAT_subset.tail(total_thermistors_in_column-Th_ice)
            #print(df_TEMP_subset.stdev)
            rank=df_HEAT_subset.sort_values(by=['stdev'])
            #print(rank)
            rank=rank.head(10)
            stdevslow=rank['stdev'].values.tolist()
            stdevice=sum(stdevslow)/len(stdevslow)
            ice_thermistors=ice_or_water_thermistors.where(ice_or_water_thermistors.stdev<stdevice+(stdevice/10))
            ice_thermistors=ice_thermistors.dropna()
            Th_ocean=len(ice_thermistors)+Th_ice
            #Th_ocean=ice_thermistors.tail(2)
            #Th_ocean=list(Th_ocean.index.values)
            #Th_ocean=int(Th_ocean[0])
...
T78=Buoy('T78'
         ,'https://data.seaiceportal.de/download/buoys/2020T78_300234068529570_TEMP_proc.csv',
         'https://data.seaiceportal.de/download/buoys/2020T78_300234068529570_HEAT030_proc.csv'
        ,'https://data.seaiceportal.de/download/buoys/2020T78_300234068529570_TS.csv',        
        '2020-11-01')
